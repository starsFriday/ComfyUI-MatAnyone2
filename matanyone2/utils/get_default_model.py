...
def get_matanyone2_model(ckpt_path, device=None) -> MatAnyone2:
    # Hydra is global-state based. Clear previous state so repeated ComfyUI runs
    # do not fail with "GlobalHydra is already initialized".
    if GlobalHydra.instance().is_initialized():
        GlobalHydra.instance().clear()

    with initialize(version_base='1.3.2', config_path="../config", job_name="eval_our_config"):
